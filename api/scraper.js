...
module.exports = class Scraper {
  parseResult(html) {
    ...
    for (i = 0; i < count; i += 1) {
      ...
      dicEntry.pos = $(title).find('em').eq(0).text()
        .replace(/\s+/g, ' ')
        .trim();

      dicEntry.transPos = $(title).find('em').eq(1).text()
        .replace(/\s+/g, ' ')
        .trim();
      ...
      for (j = 0; j < senses.length; j += 1) {
        ...
        sense.title = current.eq(0).text()
          .replace(/\s+/g, ' ')
          .replace(/\d*\.\d*/, '')
          .trim();

        sense.def = current.eq(1).text().replace(/\s+/g, ' ').trim();
        sense.trans = current.eq(2).text().replace(/\s+/g, ' ').trim();
  ...
  searchWords(q) {
    this.url = `https://krdict.korean.go.kr/eng/dicSearch/search?nation=eng&sort=C&nationCode=6&ParaWordNo=&blockCount=5&mainSearchWord=${q}`;
