    ...
    async def select_callback(self, select: ui.Select, interaction):
        if select.values[0] == "invite":
            await interaction.response.send_message(
                f"Either click the 'Add to Server' button on my profile or use <{settings['invite_link']}> to invite me to your server.",
                ephemeral=True,
            )
        elif select.values[0] == "support":
            await interaction.response.send_message(
                f"Use {settings['support_discord']} for any questions and inquiries regarding Sejong. Feel free to mention <@!{settings['owner_id']}> :)",
                ephemeral=True,
            )
